	...
	<script>
		...
		function doSubmit(){//回调函数，在编辑和保存动作时，供openDialog调用提交表单。
			...
			$("#contentTable tbody tr").each(function(){
				var cardid = $(this).find("td").eq(0).find("input").attr("id");
...
<body class="gray-bg">
	...
	<form:form id="inputForm" modelAttribute="dmCardOrder" action="${ctx}/dmcardorder/dmCardOrder/piliangFahuoSave" method="post" class="form-horizontal">
	...
	<div class="col-sm-12">
		<div class="pull-left">
			...
			<shiro:hasPermission name="dmcardorder:dmCardOrder:add">
				<table:addRow url="${ctx}/dmcardorder/dmCardOrder/form" title="菜品订单" width="60%" height="80%"></table:addRow><!-- 增加按钮 -->
			</shiro:hasPermission>
			<div hidden="hidden">
				<shiro:hasPermission name="dmcardorder:dmCardOrder:edit">
				    <table:editRow url="${ctx}/dmcardorder/dmCardOrder/form" title="菜品订单" id="contentTable" width="60%" height="80%"></table:editRow><!-- 编辑按钮 -->
				</shiro:hasPermission>
			</div>
	...
	<table id="contentTable" class="table table-striped table-bordered table-hover table-condensed dataTables-example dataTable">
		<thead>
			<tr>
				...
				<th  class="sort-column memberName">购买会员</th>
		...
		<tbody>
		<c:forEach items="${lists}" var="dmCardOrder">
			<tr>
				
				<td> <input type="checkbox" name="id" id="${dmCardOrder.id}" class="i-checks"></td>
				<td>
					${dmCardOrder.ddh}<br/>
				</td>
				<td>

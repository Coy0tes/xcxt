...
<head>
	...
	$(document).ready(function(){
	    $("#select_id").change(function(){
	    	...
	    	$.ajax({
	    		...
	    		success:function(data){
	    			if(data.code=="11"){
	    				isPackageRepeat = false;
	    				top.layer.msg(data.msg,{icon:2});
	    			}else{
	    				isPackageRepeat = true;
	    			}
	    		}
	    	});
	    	...
	    	// 获得对应套餐的子表
	    	$.ajax({
	    		url:ctx + '/xuancai/xuancai/getPackageGoods',
	    		data:{packageid:id},
	    		dataType:'json',
	    		type:'post',
	    		async:false,
	    		success:function(data){
	   				$("#dmPackageGoodsZengsongList").empty();
	   				
	   				$(data.dmZengsong).each(function(d,a){
	   					$("#dmPackageGoodsZengsongList").append("<tr><td>" + a.name + " | " + a.guige + "</td></tr>");
	   				});
	    		}
	    	});
	...
	function doSubmit(){//回调函数，在编辑和保存动作时，供openDialog调用提交表单。11
		...
		$("#xuancaiGoodsList tr").find("input[id='goodsid']").each(function(){	// 定位存在id的行
			...
			var delflag = $(this).closest('tr').find("td").eq(0).find("input").eq(1).val();
			if(delflag!="1"){
				arr.push(ids);
			}
		});
...
// -------------------------autocomplete自动补全------------------------------
	...
	function delRow(obj, prefix){
		var id = $( prefix+" #goodsid");	// id是子表的id
	...
	<form:form id="inputForm" modelAttribute="xuancai" action="${ctx}/xuancai/xuancai/save" method="post" class="form-horizontal">
		...
		<table class="table table-bordered  table-condensed dataTables-example dataTable no-footer">
		   <tbody>
				...
				<tr>
					...
					<td class="width-35">
						<form:radiobuttons id="isch" path="ison" items="${fns:getDictList('yes_no')}" itemLabel="label" itemValue="value" htmlEscape="false" class="i-checks required"/>
					</td>
					<td class="width-15 active"><label class="pull-right"><font color="red">*</font>是否推荐显示：</label></td>
					<td class="width-35">
						<form:radiobuttons path="tuijian" items="${fns:getDictList('yes_no')}" itemLabel="label" itemValue="value" htmlEscape="false" class="i-checks required"/>
					</td>
				</tr>
				<tr>
					<td class="width-15 active"><label class="pull-right"><font color="red">*</font>选菜开始时间：</label></td>
					<td class="width-35">
						<form:input path="begindate" htmlEscape="false"  onclick="WdatePicker({dateFmt:'yyyy-MM-dd H:mm'})"  onfocus="this.blur()"  class="form-control required" />
					</td>
					<td class="width-15 active"><label class="pull-right"><font color="red">*</font>选菜截止时间：</label></td>
					<td class="width-35">
						<form:input path="enddate" htmlEscape="false"  onclick="WdatePicker({dateFmt:'yyyy-MM-dd H:mm'})" onfocus="this.blur()" class="form-control required" />
					</td>
		  		</tr>
		 	</tbody>
		</table>
		...
		<div class="tabs-container">
            <ul class="nav nav-tabs">
				<li class="active"><a data-toggle="tab" href="#tabs-1" aria-expanded="true">选菜规格：</a></li>
				<li class=""><a data-toggle="tab" href="#tabs-2" aria-expanded="false">套餐固定菜品：</a></li>
            </ul>
			...
			<div id="tabs-1" class="tab-pane active">
				...
				<tr id="xuancaiGoodsList{{idx}}">
					...
					<td>
						<input id="xuancaiGoodsList{{idx}}_num" name="xuancaiGoodsList[{{idx}}].num" type="text" value="{{row.num}}"  class="form-control "/>
					</td>
			...
			<table id="contentTable" class="table table-striped table-bordered table-condensed"">
				...
				<c:forEach items="${dmZengsong }" var="dm">
					<tr>
						<td>${dm.name } | ${dm.guige}</td>
					</tr>
				</c:forEach>
				</tbody>
			</table>
